...
load(here::here("data", "core", "data_CT_encounter_itemgrp.RData"))
#load(here::here("data", "data_baseline.RData"))

data_canon <- data_CT_encounter_itemgrp %>%
  filter(!is.na(score), !(ptid %in% c("P02029","P02065", "P104"))) %>%
  mutate(itemgrp = factor(itemgrp, levels=c("Anxiety","Delusions", "Depression", "Hallucinations")))


data_wide <-
  data_canon %>%
  select(ptid, fup_day=fup_days, item=itemgrp, response = score) %>%

  group_by(ptid, fup_day) %>%
  spread(item, response) %>%
  mutate(fup_day_index = ceiling(fup_day)) %>%
  group_by(ptid) %>%
  mutate(
  ...
  arrange(ptid, fup_day) %>%
  ungroup() #%>%


dropouts <- data_canon %>% group_by(ptid) %>%
  summarise(maxfup=max(ceiling(fup_days)),
            earlyDO = maxfup<(12*7)-7
            )

sum(dropouts$earlyDO)


# response distribution ---------------------------------------------------

hmm_333 <- readRDS(file = here::here("hmm_3_3_3.rds"))
hmm_223 <- readRDS(file = here::here("hmm_2_2_3.rds"))
...
ggsave(here::here("markdown","generic EMA app", "robjects", "fig_respdistr.png"), plot=plot_respdistr)
...
plot_ppline <-
  ...
  NULL
ggsave(here::here("markdown","generic EMA app", "robjects", "fig_ppline.png"), plot=plot_ppline)
...
plot_qqline <-
  ...
  NULL
ggsave(here::here("markdown","generic EMA app", "robjects", "fig_qqline.png"), plot=plot_qqline)
...
prevalence.msm(hmm_223$hmmfit[[1]])
?prevalence.msm


expected.prev1 <- expm::expm(1000*qmatrix.msm(hmm_223$hmmfit[[1]])$estimates)[1,1:2]
expected.prev2 <- expm::expm(1000*qmatrix.msm(hmm_223$hmmfit[[1]])$estimates)[3,3:4]
expected.prev3 <- expm::expm(1000*qmatrix.msm(hmm_223$hmmfit[[1]])$estimates)[5,5:7]

fbprobs <- viterbi.msm(hmm_223$hmmfit[[1]])$pstate; fbprobs[is.na(fbprobs)]<-0

observed.prev1 <- colMeans(fbprobs[,1:2]); observed.prev1 <- observed.prev1/sum(observed.prev1)
observed.prev2 <- colMeans(fbprobs[,3:4]); observed.prev2 <- observed.prev2/sum(observed.prev2)
observed.prev3 <- colMeans(fbprobs[,5:7]); observed.prev3 <- observed.prev3/sum(observed.prev3)

observed.count1 <- colSums(fbprobs[,1:2])
observed.count2 <- colSums(fbprobs[,3:4])
observed.count3 <- colSums(fbprobs[,5:7])

# chisq.test(observed.count1, p = expected.prev1)
# chisq.test(observed.count2, p = expected.prev2)
# chisq.test(observed.count3, p = expected.prev3)



# BICs --------------------------------------------------------------------

BICs <- readRDS(file=here::here("data", "simulations", "BICs.rds"))
...
crossing(
  ...
  mutate(
    ...
    n.par.Phi = map_dbl(n.states_cluster, ~sum(.*(.-1))),
    n.par.phi1 = n.states-1,
    n.par.Theta_cat = 24*n.states,
    n.par.Theta_ord = map_dbl(n.states_cluster, ~sum(24 + 4*(. - 1))),
    n.par.cat = n.par.Phi + n.par.phi1 + n.par.Theta_cat,
    n.par.ord = n.par.Phi + n.par.phi1 + n.par.Theta_ord,
    n.par.Theta = if_else(type=="categorical", n.par.Theta_cat, n.par.Theta_ord),
    n.par = if_else(type=="categorical", n.par.cat, n.par.ord),
  ) %>%
  arrange(type, n.clusters, n.states) %>%
  filter(n.states <=9, n.states != n.clusters, n.clusters %in% c(1,2,3), map_lgl(n.states_cluster, ~!any(. %ni% c(2,3))) ) %>%
  left_join(BICs) %>%
  mutate(
    BIC = if_else(type=="categorical", BICcat, BICord),
    BICnonmiss = if_else(type=="categorical", BICcat.nonmiss, BICord.nonmiss),
  )

saveRDS(params, here::here("markdown","generic EMA app", "robjects", "params.rds"))
...
ggsave(here::here("markdown","generic EMA app", "robjects", "sim_obsfreq.png"), plot=plot_freq, width=20, height=15, units="cm")
...
ggsave(here::here("markdown","generic EMA app", "robjects", "sim_obsfreqalert.png"), plot=plot_freq, width=20, height=15, units="cm")
...
ggsave(here::here("markdown","generic EMA app", "robjects", "sim_obsfreqnotalert.png"), plot=plot_freq, width=20, height=15, units="cm")


freqplots <- cowplot::plot_grid(plot_freq, plot_freqalert, plot_freqnotalert, ncol=1, axis="rl", align="hv")

ggsave(here::here("markdown","generic EMA app", "robjects", "freqplots.png"), plot=freqplots, width=20, height=20, units="cm")
...
ggsave(here::here("markdown","generic EMA app", "robjects", "sim_avgdelay.png"), plot=plot_avgdelay, width=20, height=15, units="cm")
...
ggsave(here::here("markdown","generic EMA app", "robjects", "sim_delay.png"), plot=plot_delay, width=20, height=15, units="cm")
...
ggsave(here::here("markdown","generic EMA app", "robjects", "sim_interval.png"), plot=plot_interval, width=20, height=15, units="cm")
...
ggsave(here::here("markdown","generic EMA app", "robjects", "sim_transitions.png"), plot=plot_nbadtransitions, width=20, height=15, units="cm")
...
plot_maxdelay <-
  ...
  facet_grid(rows=vars(cluster),
             scales="free_y")+
  geom_density(aes(x=maxdelay, colour=as.factor(tau)), size=1, alpha=0.6)+
  scale_y_continuous(expand = c(0, 0)) +
  #scale_color_brewer(type='qual', direction=-1)+
  scale_colour_manual(values=viridisLite::viridis(n=6), na.value="transparent", labels=c("0", "2^-5", "2^-4","2^-3","2^-2","2^-1"), drop = FALSE)+
  labs(x='Maximum delay (days)', colour='tau')+
  theme_bw()

ggsave(here::here("markdown","generic EMA app", "robjects", "sim_maxdelay.png"), plot=plot_maxdelay, width=20, height=15, units="cm")
...
saveRDS(results1table, here::here("markdown","generic EMA app", "robjects", "results1table.rds"))
...
ggsave(here::here("markdown","generic EMA app", "robjects", "barplots.png"), plot=barplots, width=20, height=20, units="cm")


# real data results -------------------------------------------------------


realresults <- readRDS(file=here::here("data", "simulations", "realresults_223.rds"))
...
ggsave(here::here("markdown","generic EMA app", "robjects", "plot_obsused.png"), plot=plot_used, width=20, height=20, units="cm")





filter(data_wide, ptid=="P02059") %>%view()
